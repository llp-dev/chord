...
Thanks to the [`nb`](../nb/index.md) crate the HAL API can be used in a blocking manner
...
|------|------|-------------|
| [`serial`](#serial) | mod | Serial interface. |
| [`spi`](#spi) | mod | SPI master mode traits using `nb`. |

## Modules

- [`serial`](serial/index.md) — Serial interface.
- [`spi`](spi/index.md) — SPI master mode traits using `nb`.
