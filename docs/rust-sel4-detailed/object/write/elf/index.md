*[object](../../index.md) / [write](../index.md) / [elf](index.md)*
...
Provides [`Writer`](#writer) for low level writing of ELF files.
...
|------|------|-------------|
| [`object`](#object) | mod |  |
| [`writer`](#writer) | mod | Helper for writing ELF files. |
...
- [`writer`](writer/index.md) — Helper for writing ELF files.
...
- <span id="sectionindex-default"></span>`fn default() -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="sectionindex-ord-cmp"></span>`fn cmp(&self, other: &SectionIndex) -> cmp::Ordering` — [`SectionIndex`](#sectionindex)
...
- <span id="sectionindex-partialord-partial-cmp"></span>`fn partial_cmp(&self, other: &SectionIndex) -> option::Option<cmp::Ordering>` — [`SectionIndex`](#sectionindex)
...
- <span id="symbolindex-default"></span>`fn default() -> SymbolIndex` — [`SymbolIndex`](#symbolindex)
...
- <span id="symbolindex-ord-cmp"></span>`fn cmp(&self, other: &SymbolIndex) -> cmp::Ordering` — [`SymbolIndex`](#symbolindex)
...
- <span id="symbolindex-partialord-partial-cmp"></span>`fn partial_cmp(&self, other: &SymbolIndex) -> option::Option<cmp::Ordering>` — [`SymbolIndex`](#symbolindex)
...
- <span id="writer-class"></span>`fn class(&self) -> Class` — [`Class`](#class)
...
- <span id="writer-write-program-header"></span>`fn write_program_header(&mut self, header: &ProgramHeader)` — [`ProgramHeader`](#programheader)

  Write a program header.

- <span id="writer-reserve-null-section-index"></span>`fn reserve_null_section_index(&mut self) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-reserve-section-index"></span>`fn reserve_section_index(&mut self) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-reserve-shstrtab-section-index"></span>`fn reserve_shstrtab_section_index(&mut self) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-reserve-shstrtab-section-index-with-name"></span>`fn reserve_shstrtab_section_index_with_name(&mut self, name: &'a [u8]) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-reserve-strtab-section-index"></span>`fn reserve_strtab_section_index(&mut self) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-reserve-strtab-section-index-with-name"></span>`fn reserve_strtab_section_index_with_name(&mut self, name: &'a [u8]) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-reserve-null-symbol-index"></span>`fn reserve_null_symbol_index(&mut self) -> SymbolIndex` — [`SymbolIndex`](#symbolindex)
...
- <span id="writer-reserve-symbol-index"></span>`fn reserve_symbol_index(&mut self, section_index: Option<SectionIndex>) -> SymbolIndex` — [`SectionIndex`](#sectionindex), [`SymbolIndex`](#symbolindex)
...
- <span id="writer-write-symbol"></span>`fn write_symbol(&mut self, sym: &Sym)` — [`Sym`](#sym)

  Write a symbol.

- <span id="writer-reserve-symtab-section-index"></span>`fn reserve_symtab_section_index(&mut self) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-reserve-symtab-section-index-with-name"></span>`fn reserve_symtab_section_index_with_name(&mut self, name: &'a [u8]) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-symtab-index"></span>`fn symtab_index(&mut self) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-reserve-symtab-shndx-section-index"></span>`fn reserve_symtab_shndx_section_index(&mut self) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-reserve-symtab-shndx-section-index-with-name"></span>`fn reserve_symtab_shndx_section_index_with_name(&mut self, name: &'a [u8]) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-reserve-dynstr-section-index"></span>`fn reserve_dynstr_section_index(&mut self) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-reserve-dynstr-section-index-with-name"></span>`fn reserve_dynstr_section_index_with_name(&mut self, name: &'a [u8]) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-dynstr-index"></span>`fn dynstr_index(&mut self) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-reserve-null-dynamic-symbol-index"></span>`fn reserve_null_dynamic_symbol_index(&mut self) -> SymbolIndex` — [`SymbolIndex`](#symbolindex)
...
- <span id="writer-reserve-dynamic-symbol-index"></span>`fn reserve_dynamic_symbol_index(&mut self) -> SymbolIndex` — [`SymbolIndex`](#symbolindex)
...
- <span id="writer-write-dynamic-symbol"></span>`fn write_dynamic_symbol(&mut self, sym: &Sym)` — [`Sym`](#sym)

  Write a dynamic symbol.

- <span id="writer-reserve-dynsym-section-index"></span>`fn reserve_dynsym_section_index(&mut self) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-reserve-dynsym-section-index-with-name"></span>`fn reserve_dynsym_section_index_with_name(&mut self, name: &'a [u8]) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-dynsym-index"></span>`fn dynsym_index(&mut self) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-reserve-dynamic-section-index"></span>`fn reserve_dynamic_section_index(&mut self) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-reserve-hash-section-index"></span>`fn reserve_hash_section_index(&mut self) -> SectionIndex` — [`SectionIndex`](#sectionindex)

  Reserve the section index for the SysV hash table.

- <span id="writer-reserve-hash-section-index-with-name"></span>`fn reserve_hash_section_index_with_name(&mut self, name: &'a [u8]) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-reserve-gnu-hash-section-index"></span>`fn reserve_gnu_hash_section_index(&mut self) -> SectionIndex` — [`SectionIndex`](#sectionindex)

  Reserve the section index for the GNU hash table.

- <span id="writer-reserve-gnu-hash-section-index-with-name"></span>`fn reserve_gnu_hash_section_index_with_name(&mut self, name: &'a [u8]) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-reserve-gnu-versym-section-index"></span>`fn reserve_gnu_versym_section_index(&mut self) -> SectionIndex` — [`SectionIndex`](#sectionindex)

  Reserve the section index for the `.gnu.version` section.

- <span id="writer-reserve-gnu-versym-section-index-with-name"></span>`fn reserve_gnu_versym_section_index_with_name(&mut self, name: &'a [u8]) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-write-gnu-verdef"></span>`fn write_gnu_verdef(&mut self, verdef: &Verdef)` — [`Verdef`](#verdef)

  Write a version definition entry.

- <span id="writer-write-gnu-verdef-shared"></span>`fn write_gnu_verdef_shared(&mut self, verdef: &Verdef)` — [`Verdef`](#verdef)
...
- <span id="writer-reserve-gnu-verdef-section-index"></span>`fn reserve_gnu_verdef_section_index(&mut self) -> SectionIndex` — [`SectionIndex`](#sectionindex)

  Reserve the section index for the `.gnu.version_d` section.

- <span id="writer-reserve-gnu-verdef-section-index-with-name"></span>`fn reserve_gnu_verdef_section_index_with_name(&mut self, name: &'a [u8]) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-write-gnu-verneed"></span>`fn write_gnu_verneed(&mut self, verneed: &Verneed)` — [`Verneed`](#verneed)

  Write a version need entry.

- <span id="writer-write-gnu-vernaux"></span>`fn write_gnu_vernaux(&mut self, vernaux: &Vernaux)` — [`Vernaux`](#vernaux)

  Write a version need auxiliary entry.

- <span id="writer-reserve-gnu-verneed-section-index"></span>`fn reserve_gnu_verneed_section_index(&mut self) -> SectionIndex` — [`SectionIndex`](#sectionindex)

  Reserve the section index for the `.gnu.version_r` section.

- <span id="writer-reserve-gnu-verneed-section-index-with-name"></span>`fn reserve_gnu_verneed_section_index_with_name(&mut self, name: &'a [u8]) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-reserve-gnu-attributes-section-index"></span>`fn reserve_gnu_attributes_section_index(&mut self) -> SectionIndex` — [`SectionIndex`](#sectionindex)

  Reserve the section index for the `.gnu.attributes` section.

- <span id="writer-reserve-gnu-attributes-section-index-with-name"></span>`fn reserve_gnu_attributes_section_index_with_name(&mut self, name: &'a [u8]) -> SectionIndex` — [`SectionIndex`](#sectionindex)
...
- <span id="writer-write-relocation"></span>`fn write_relocation(&mut self, is_rela: bool, rel: &Rel)` — [`Rel`](#rel)

  Write a relocation.

- <span id="writer-write-relocation-section-header"></span>`fn write_relocation_section_header(&mut self, name: StringId, section: SectionIndex, symtab: SectionIndex, offset: usize, count: usize, is_rela: bool)` — [`StringId`](../string/index.md#stringid), [`SectionIndex`](#sectionindex)
...
- <span id="writer-write-comdat-entry"></span>`fn write_comdat_entry(&mut self, entry: SectionIndex)` — [`SectionIndex`](#sectionindex)

  Write an entry in a COMDAT section.

- <span id="writer-write-comdat-section-header"></span>`fn write_comdat_section_header(&mut self, name: StringId, symtab: SectionIndex, symbol: SymbolIndex, offset: usize, count: usize)` — [`StringId`](../string/index.md#stringid), [`SectionIndex`](#sectionindex), [`SymbolIndex`](#symbolindex)

  Write the section header for a COMDAT section.

- <span id="writer-attributes-writer"></span>`fn attributes_writer(&self) -> AttributesWriter` — [`AttributesWriter`](#attributeswriter)
...
- <span id="class-default"></span>`fn default() -> Class` — [`Class`](#class)
...
- <span id="sym-clone"></span>`fn clone(&self) -> Sym` — [`Sym`](#sym)
...
- <span id="rel-clone"></span>`fn clone(&self) -> Rel` — [`Rel`](#rel)
...
- <span id="verdef-clone"></span>`fn clone(&self) -> Verdef` — [`Verdef`](#verdef)
...
- <span id="verneed-clone"></span>`fn clone(&self) -> Verneed` — [`Verneed`](#verneed)
...
- <span id="vernaux-clone"></span>`fn clone(&self) -> Vernaux` — [`Vernaux`](#vernaux)
