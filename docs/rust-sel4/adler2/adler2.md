...
- [`Adler32`](#adler32) - Adler-32 checksum calculator.

**Functions**

- [`adler32_slice`](#adler32_slice) - Calculates the Adler-32 checksum of a byte slice.
...
- `fn new() -> Self` - Creates a new Adler-32 instance with default state.
- `fn from_checksum(sum: u32) -> Self` - Creates an `Adler32` instance from a precomputed Adler-32 checksum.
- `fn checksum(self: &Self) -> u32` - Returns the calculated checksum at this point in time.
- `fn write_slice(self: & mut Self, bytes: &[u8])` - Adds `bytes` to the checksum calculation.
