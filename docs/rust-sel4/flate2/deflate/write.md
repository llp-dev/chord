**flate2 > deflate > write**

# Module: deflate::write
...
## flate2::deflate::write::DeflateDecoder
...
- `fn new(w: W) -> DeflateDecoder<W>` - Creates a new decoder which will write uncompressed data to the stream.
...
- `fn new(w: W, level: crate::Compression) -> DeflateEncoder<W>` - Creates a new encoder which will write compressed data to the stream
- `fn get_ref(self: &Self) -> &W` - Acquires a reference to the underlying writer.
- `fn get_mut(self: & mut Self) -> & mut W` - Acquires a mutable reference to the underlying writer.
